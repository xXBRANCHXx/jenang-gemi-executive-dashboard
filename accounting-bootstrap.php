...
function jg_accounting_ensure_schema(PDO $pdo): void
{
    $statements = [
        ...
        'CREATE TABLE IF NOT EXISTS accounting_import_state (
...
function jg_accounting_config_int(string $envKey, string $configKey, int $default, int $min, int $max): int
{
    $config = jg_dashboard_load_local_config();
    $raw = jg_dashboard_env_value($envKey);
    if ($raw === '' && array_key_exists($configKey, $config)) {
...
function jg_accounting_config_text(string $envKey, string $configKey, string $default = ''): string
{
    $config = jg_dashboard_load_local_config();
    $raw = jg_dashboard_env_value($envKey);
    if ($raw === '' && array_key_exists($configKey, $config)) {
...
function jg_accounting_wallet_sync_accounts(): array
{
    $cashAccountKey = jg_accounting_config_text('JG_ACCOUNTING_WALLET_CASH_ACCOUNT_KEY', 'accounting_wallet_cash_account_key', 'bca-main');
    $defaultAccounts = [
        [
            'platform' => 'shopee',
            'account_key' => 'jenang-gemi-shopee',
            'wallet_account_key' => 'shopee-jg-wallet',
            'cash_account_key' => $cashAccountKey,
            'label' => 'Shopee Wallet - Jenang Gemi',
        ],
        [
            'platform' => 'shopee',
            'account_key' => 'zero-shopee',
            'wallet_account_key' => 'shopee-zero-wallet',
            'cash_account_key' => $cashAccountKey,
            'label' => 'Shopee Wallet - ZERO',
        ],
    ];

    $config = jg_dashboard_load_local_config();
    $configured = $config['accounting_wallet_sync_accounts'] ?? null;
    ...
    foreach ($configured as $row) {
        ...
        $platform = jg_accounting_text($row['platform'] ?? '', 40);
        $accountKey = jg_accounting_text($row['account_key'] ?? '', 80);
        $walletAccountKey = jg_accounting_text($row['wallet_account_key'] ?? '', 80);
        if ($platform === '' || $accountKey === '' || $walletAccountKey === '') {
            continue;
        }
        $accounts[] = [
            'platform' => $platform,
            'account_key' => $accountKey,
            'wallet_account_key' => $walletAccountKey,
            'cash_account_key' => jg_accounting_text($row['cash_account_key'] ?? $cashAccountKey, 80),
            'label' => jg_accounting_text($row['label'] ?? $walletAccountKey, 160),
        ];
    }

    return $accounts !== [] ? $accounts : $defaultAccounts;
}

function jg_accounting_import_state(PDO $pdo, string $key): ?array
{
    $stmt = $pdo->prepare('SELECT state_value, updated_at FROM accounting_import_state WHERE state_key = :state_key LIMIT 1');
...
function jg_accounting_import_state_is_fresh(?array $state, int $ttlSeconds): bool
{
    if (!is_array($state) || $ttlSeconds <= 0) {
        return false;
    }
    $updatedAt = strtotime((string) ($state['updated_at'] ?? '') . ' UTC');
    return $updatedAt !== false && $updatedAt >= time() - $ttlSeconds;
}

function jg_accounting_touch_import_state(PDO $pdo, string $key, array $value): void
{
    $stmt = $pdo->prepare(
        'INSERT INTO accounting_import_state (state_key, state_value, updated_at)
         VALUES (:state_key, :state_value, UTC_TIMESTAMP())
         ON DUPLICATE KEY UPDATE state_value = VALUES(state_value), updated_at = UTC_TIMESTAMP()'
    );
    $stmt->execute([
        ':state_key' => $key,
        ':state_value' => json_encode($value, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE),
    ]);
}

function jg_accounting_wallet_sync_start_date(): string
{
    $explicitStart = jg_accounting_config_text('JG_ACCOUNTING_WALLET_SYNC_START_DATE', 'accounting_wallet_sync_start_date');
    if ($explicitStart !== '' && preg_match('/^\d{4}-\d{2}-\d{2}$/', $explicitStart)) {
        return $explicitStart;
    }

    $days = jg_accounting_config_int('JG_ACCOUNTING_WALLET_SYNC_DAYS', 'accounting_wallet_sync_days', 45, 1, 366);
    return jg_accounting_now()->modify('-' . $days . ' days')->format('Y-m-d');
}

function jg_accounting_fetch_wallet_transactions(string $platform, string $accountKey, string $startDate, string $endDate): array
{
    ...
    $url = jg_dashboard_marketplace_api_base_url()
        . '/sales/wallet-transactions?'
        . http_build_query($query, '', '&', PHP_QUERY_RFC3986);
    $timeout = jg_accounting_config_int('JG_ACCOUNTING_WALLET_SYNC_TIMEOUT_SECONDS', 'accounting_wallet_sync_timeout_seconds', 12, 2, 45);
    ...
    if (!is_string($response) || $response === '') {
        throw new RuntimeException('Wallet transaction endpoint was unreachable.');
    }
    ...
    if ($status < 200 || $status >= 300) {
        throw new RuntimeException('Wallet transaction endpoint returned HTTP ' . $status . '.');
    }

    $payload = json_decode($response, true);
    if (!is_array($payload)) {
        throw new RuntimeException('Wallet transaction endpoint returned invalid JSON.');
    }
    return $payload;
}

function jg_accounting_wallet_payload_transactions(array $payload): array
{
    $accounts = $payload['wallet_transactions']['accounts'] ?? [];
    ...
    foreach ($accounts as $account) {
        if (!is_array($account) || !is_array($account['transactions'] ?? null)) {
            continue;
        }
...
function jg_accounting_wallet_payload_latest_balance(array $payload): ?int
{
    $latestTimestamp = null;
    $latestBalance = null;
    foreach (jg_accounting_wallet_payload_transactions($payload) as $transaction) {
        if (!array_key_exists('current_balance', $transaction) || $transaction['current_balance'] === null || !is_numeric($transaction['current_balance'])) {
            continue;
        }
        $timestamp = (int) ($transaction['create_time'] ?? 0);
        if ($latestTimestamp === null || $timestamp >= $latestTimestamp) {
            $latestTimestamp = $timestamp;
            $latestBalance = (int) round((float) $transaction['current_balance']);
        }
    }
    return $latestBalance;
}

function jg_accounting_wallet_row_amount(array $row): int
{
    return (int) round((float) ($row['amount'] ?? 0));
}

function jg_accounting_wallet_row_search_text(array $row): string
{
...
function jg_accounting_is_wallet_payout_row(array $row): bool
{
    if (jg_accounting_wallet_row_amount($row) >= 0) {
        return false;
    }

    $text = jg_accounting_wallet_row_search_text($row);
    ...
    $orderId = trim((string) ($row['order_id'] ?? $row['order_sn'] ?? ''));
    return $orderId === '' && preg_match('/\b(out|debit|withdraw)\b/i', $text) === 1;
}

function jg_accounting_wallet_row_date(array $row): string
{
    $timestamp = (int) ($row['create_time'] ?? 0);
    if ($timestamp > 0) {
    ...
    return jg_accounting_now()->format('Y-m-d');
}

function jg_accounting_account_id_by_key(PDO $pdo, string $accountKey): ?int
{
    $stmt = $pdo->prepare('SELECT id FROM accounting_accounts WHERE account_key = :account_key AND is_active = 1 LIMIT 1');
    $stmt->execute([':account_key' => $accountKey]);
    $id = $stmt->fetchColumn();
    return $id === false ? null : (int) $id;
}

function jg_accounting_wallet_reference(string $platform, string $accountKey, array $row): string
{
    $transactionId = trim((string) ($row['transaction_id'] ?? ''));
    if ($transactionId === '') {
        $transactionId = hash('sha256', json_encode($row, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) ?: serialize($row));
    }
    return mb_substr('wallet:' . $platform . ':' . $accountKey . ':' . hash('sha256', $transactionId), 0, 160);
}

function jg_accounting_wallet_transfer_exists(PDO $pdo, int $walletAccountId, int $cashAccountId, string $date, int $amount, string $referenceNo): bool
{
    ...
    $referenceStmt->execute([':reference_no' => $referenceNo]);
    ...
    return $manualStmt->fetchColumn() !== false;
}

function jg_accounting_import_wallet_payout(PDO $pdo, array $account, array $row): string
{
    $walletAccountId = jg_accounting_account_id_by_key($pdo, (string) $account['wallet_account_key']);
    $cashAccountId = jg_accounting_account_id_by_key($pdo, (string) $account['cash_account_key']);
    if ($walletAccountId === null || $cashAccountId === null) {
        return 'missing_account';
    }

    $amount = abs(jg_accounting_wallet_row_amount($row));
    if ($amount <= 0) {
        return 'ignored';
    }

    $date = jg_accounting_wallet_row_date($row);
    $platform = (string) $account['platform'];
    $accountKey = (string) $account['account_key'];
    $referenceNo = jg_accounting_wallet_reference($platform, $accountKey, $row);
    if (jg_accounting_wallet_transfer_exists($pdo, $walletAccountId, $cashAccountId, $date, $amount, $referenceNo)) {
        return 'matched';
    }

    $transactionType = jg_accounting_text($row['transaction_type'] ?? '', 80);
    $walletTransactionId = jg_accounting_text($row['transaction_id'] ?? '', 120);
    $description = 'Imported marketplace wallet payout from ' . (string) ($account['label'] ?? $accountKey);
    $notes = trim(implode(' ', array_filter([
        $walletTransactionId !== '' ? 'Wallet transaction ' . $walletTransactionId . '.' : '',
        $transactionType !== '' ? 'Type: ' . $transactionType . '.' : '',
    ])));

    $payload = [
        ':transaction_key' => jg_accounting_key('wallet'),
        ':transaction_date' => $date,
        ...
        ':category_id' => null,
        ':bill_id' => null,
        ':brand' => jg_accounting_text($account['label'] ?? '', 80),
        ':channel' => $platform,
        ':amount' => $amount,
        ':transfer_fee_amount' => 0,
        ':payment_method' => 'Marketplace Wallet',
        ':reference_no' => $referenceNo,
        ':invoice_no' => '',
        ':order_no' => trim((string) ($row['order_id'] ?? '')),
        ':receipt_url' => '',
    ...
    jg_accounting_insert_audit($pdo, 'transaction', $id, 'wallet_payout_import', null, $payload);
...
function jg_accounting_marketplace_outstanding_context(): array
{
    return [
        ...
        'accounts' => [],
        'errors' => [],
        'wallet_payouts_imported_amount' => 0,
        'wallet_payouts_imported_count' => 0,
        'wallet_payouts_matched_count' => 0,
    ];
}

function jg_accounting_sync_marketplace_wallet_context(PDO $pdo): array
{
    $stateKey = 'marketplace_wallet_payout_sync';
    $ttlSeconds = jg_accounting_config_int('JG_ACCOUNTING_WALLET_SYNC_TTL_SECONDS', 'accounting_wallet_sync_ttl_seconds', 300, 0, 3600);
    $cachedState = jg_accounting_import_state($pdo, $stateKey);
    if (jg_accounting_import_state_is_fresh($cachedState, $ttlSeconds)) {
        return array_merge(jg_accounting_marketplace_outstanding_context(), $cachedState['value'], ['cached' => true]);
    }

    $context = jg_accounting_marketplace_outstanding_context();
    $context['source'] = 'wallet_transactions';
    $context['label'] = 'Live marketplace wallet balance';
    $context['cached'] = false;
    $context['start_date'] = jg_accounting_wallet_sync_start_date();
    $context['end_date'] = jg_accounting_now()->format('Y-m-d');

    $walletBalanceTotal = 0;
    $hasWalletBalance = false;

    foreach (jg_accounting_wallet_sync_accounts() as $account) {
        $accountContext = [
            'platform' => (string) $account['platform'],
            'account_key' => (string) $account['account_key'],
            'label' => (string) $account['label'],
            'current_balance' => null,
            'payouts_imported_count' => 0,
            'payouts_imported_amount' => 0,
            'payouts_matched_count' => 0,
        ];

        try {
            $payload = jg_accounting_fetch_wallet_transactions(
                (string) $account['platform'],
                (string) $account['account_key'],
                (string) $context['start_date'],
                (string) $context['end_date']
            );
            $latestBalance = jg_accounting_wallet_payload_latest_balance($payload);
            if ($latestBalance !== null) {
                $hasWalletBalance = true;
                $walletBalanceTotal += $latestBalance;
                $accountContext['current_balance'] = $latestBalance;
            }

            foreach (jg_accounting_wallet_payload_transactions($payload) as $transaction) {
                if (!jg_accounting_is_wallet_payout_row($transaction)) {
                    continue;
                }
                $result = jg_accounting_import_wallet_payout($pdo, $account, $transaction);
                if ($result === 'imported') {
                    $amount = abs(jg_accounting_wallet_row_amount($transaction));
                    $accountContext['payouts_imported_count']++;
                    $accountContext['payouts_imported_amount'] += $amount;
                    $context['wallet_payouts_imported_count']++;
                    $context['wallet_payouts_imported_amount'] += $amount;
                } elseif ($result === 'matched') {
                    $accountContext['payouts_matched_count']++;
                    $context['wallet_payouts_matched_count']++;
                }
            }
        } catch (Throwable $error) {
            $accountContext['error'] = $error->getMessage();
            $context['errors'][] = [
                'account_key' => (string) $account['account_key'],
        ...
        $context['accounts'][] = $accountContext;
    }

    if ($hasWalletBalance) {
        $context['amount'] = $walletBalanceTotal;
    } else {
        $context['amount'] = 0;
        $context['source'] = 'wallet_context_unavailable';
        $context['label'] = 'Expected, not cash yet';
    }

    jg_accounting_touch_import_state($pdo, $stateKey, $context);
    return $context;
}
...
function jg_accounting_summary(PDO $pdo, string $month): array
{
    ...
    $marketplaceOutstanding = jg_accounting_sync_marketplace_wallet_context($pdo);
    $today = jg_accounting_now()->format('Y-m-d');
