...
{
    $partner=null;if($partnerCode!==''){$profile=$pdo->prepare('SELECT code,name,partner_class FROM partner_profiles WHERE code=:code LIMIT 1');$profile->execute([':code'=>$partnerCode]);$row=$profile->fetch();if(is_array($row))$partner=['code'=>(string)$row['code'],'name'=>(string)$row['name'],'partner_class'=>(string)$row['partner_class']];}
...
function jg_partner_stock_notifications(PDO $pdo, bool $includeHistory = false): array
{
    $events=[];$stmt=$pdo->query('SELECT id FROM partner_deposit_requests WHERE '.($includeHistory?'1=1':'status IN("pending","investigating")').' ORDER BY submitted_at DESC');foreach($stmt->fetchAll(PDO::FETCH_COLUMN) as $id){$d=jg_partner_stock_deposit($pdo,(int)$id);if($d)$events[]=['id'=>'deposit:'.$d['id'],'record_id'=>$d['id'],'type'=>'balance_deposit','partner_code'=>$d['partner_code'],'partner_name'=>$d['partner_name'],'amount'=>$d['approved_amount']??$d['requested_amount'],'created_at'=>$d['submitted_at'],'status'=>$d['status'],'action_required'=>in_array($d['status'],['pending','investigating'],true),'updated_at'=>$d['updated_at']??$d['submitted_at'],'proof'=>['url'=>$d['proof_url'],'name'=>$d['proof_name'],'mime_type'=>$d['proof_mime'],'size_bytes'=>$d['proof_size']],'detail_url'=>'/partner-stock-orders/?deposit='.$d['id'].'&partner='.rawurlencode($d['partner_code'])];}
    $stmt=$pdo->query('SELECT id FROM partner_orders WHERE order_type="class_b_stock" AND '.($includeHistory?'1=1':'executive_status="awaiting_shipment"').' ORDER BY submitted_at DESC');foreach($stmt->fetchAll(PDO::FETCH_COLUMN) as $id){$o=jg_partner_stock_order($pdo,(string)$id);if($o)$events[]=['id'=>'stock_order:'.$o['id'],'record_id'=>$o['id'],'type'=>'stock_order','status'=>$o['executive_status'],'action_required'=>$o['executive_status']==='awaiting_shipment','updated_at'=>$o['updated_at'],'partner_code'=>$o['partner_code'],'partner_name'=>$o['partner_name'],'amount'=>$o['total'],'created_at'=>$o['submitted_at'],'items'=>$o['items'],'recipient_name'=>$o['recipient_name'],'detail_url'=>'/partner-stock-orders/?order='.rawurlencode($o['id']).'&partner='.rawurlencode($o['partner_code'])];}usort($events,static fn($a,$b)=>strcmp((string)$b['created_at'],(string)$a['created_at']));return $events;
}
