...
document.addEventListener('DOMContentLoaded', () => {
    ...
    return `
      <article class="admin-notification-detail" data-notification-order="${escapeHtml(order.order_id)}">
        <div class="admin-notification-order-card">
          ...
          <div class="admin-notification-line-items">${items.map((item) => `<div><span>${escapeHtml(notificationItemLabel(item))}</span><strong>x ${Number(item.quantity || 0)}</strong><small>${escapeHtml(item.sku || '')}</small></div>`).join('')}</div>
  ...
  searchCloseButton?.addEventListener('click', (event) => {
    event.preventDefault();
    event.stopImmediatePropagation();
    closeSearchResults({ clear: true, collapse: true });
  }, true);
  ...
  searchForm?.addEventListener('submit', (event) => {
    ...
    if (!first) return;
    if (first.view) window.localStorage.setItem(viewStorageKey, first.view);
    window.location.href = first.url;
  });

  searchResults?.addEventListener('click', (event) => {
    const target = event.target instanceof Element ? event.target.closest('[data-search-view]') : null;
    if (target instanceof HTMLElement) {
      window.localStorage.setItem(viewStorageKey, target.dataset.searchView || 'home');
    }
  });
  ...
  loadNotifications().catch(() => {
    if (notificationSummary) notificationSummary.textContent = 'Order status unavailable';
  });
});
