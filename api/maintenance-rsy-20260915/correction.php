...
const RSY_ORDER = 'PO26091504BF18F3';
const RSY_DISPLAY = 'PARTNER-PO26091504BF18F3';
...
{
    return "'jg_rsy_20260915_PO26091504BF18F3','" . $branch . "'";
}
    ...
    foreach ($connections as $branch => $pdo) {
        ...
        try {
            ...
            $complete = match ($branch) {
                ...
                'inventory' => count(rsy_rows($pdo, 'SELECT id FROM direct_order_inventory_restorations WHERE order_id = ? AND sku = ? AND quantity = 40', [RSY_DISPLAY, RSY_SKU])) === 1,
...
{
    rsy_check(($body['order_id'] ?? '') === RSY_ORDER, 'This correction is restricted to the original RSY order.');
    $mode = (string) ($body['mode'] ?? '');
    rsy_check(in_array($mode, ['dry_run', 'apply'], true), 'Choose dry_run or apply.');
    $directory = sys_get_temp_dir() . '/jg-rsy-order-correction-20260915';
    ...
    try {
        ...
        rsy_check($order['partner_code'] === RSY_PARTNER && $order['customer_name'] === 'RSY'
            && $order['status'] === 'FULFILLED' && (int) $order['quantity'] === 40
            && $order['sku_code'] === RSY_SKU && (float) $order['revenue_total'] === 3816000.0
            && str_starts_with($order['created_at'], '2026-09-15')
            && $order['billing_paid_at'] === null && (float) $order['balance_amount'] === 0.0
            && count($items) === 1 && $items[0]['sku_code'] === RSY_SKU && (int) $items[0]['quantity'] === 40,
        ...
        rsy_check($billItem['status'] === 'included' && $billItem['dispute_id'] === null && $billItem['paid_at'] === null
            && (int) $billItem['units'] === 40 && (int) $billItem['amount'] === 3816000, 'Billing item changed.');
        ...
        rsy_check(count($lines) === 1 && $lines[0]['stock_sku'] === RSY_SKU && $lines[0]['selling_sku'] === RSY_SKU
            && (int) $lines[0]['base_deducted_quantity'] === 40 && (int) $lines[0]['selling_quantity'] === 40
            && (float) $lines[0]['stock_ratio'] === 1.0 && (int) $lines[0]['shortage_base_quantity'] === 0,
            'Recorded stock deduction differs from the expected 40 bottles.');
        ...
        $plan = ['order_id' => RSY_ORDER, 'partner_code' => RSY_PARTNER, 'sku' => RSY_SKU,
            'restore_quantity' => 40, 'stock_before' => (int) $sku['current_stock'], 'stock_after' => (int) $sku['current_stock'] + 40,
            'remove_revenue' => 3816000, 'remove_sales_units' => 40, 'remove_fulfilled_orders' => 1,
        ...
        rsy_write($s, 'UPDATE sku_skus SET current_stock = current_stock + 40, updated_at = UTC_TIMESTAMP() WHERE sku = ?', [RSY_SKU], 1);
        rsy_write($s, 'INSERT INTO direct_order_inventory_restorations (order_id, sku, quantity, restored_at) VALUES (?, ?, 40, UTC_TIMESTAMP())', [RSY_DISPLAY, RSY_SKU], 1);
        ...
        rsy_check((int) $afterBill['total_amount'] === (int) $snapshot['bill'][0]['total_amount'] - 3816000, 'Billing total verification failed.');
        foreach ($connections as $branch => $pdo) {
