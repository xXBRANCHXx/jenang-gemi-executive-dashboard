...
- Hard Set is initialized server-side as OFF and exposes no disable operation. The activation switch remains locked until the current session authenticates with Branch-tier SKU Database credentials and all local/remote readiness checks pass. The dashboard requires the new readiness contracts, a healthy API Ingest cron heartbeat, no unresolved fulfillment exceptions, authenticated Store Ops access, and coverage of every automatic marketplace source in the Store Ops queue. Its UTC cutover boundary, audit record, and outbox are persisted in MySQL. Delivery freezes the exact account scope in Store Ops first and activates API Ingest last, only after Store Ops echoes the same ON state, timestamp, and accounts. Retries are idempotent even if a later health check degrades, and a prominent synchronization-pending warning remains until every acknowledgement matches.
