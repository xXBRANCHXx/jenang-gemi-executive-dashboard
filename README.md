...
- Orders → Ops → Shipment Arrangement provides a Jakarta-local week/hour timetable of automatic windows, ship-by deadlines, and marketplace pickup slots. Platform and order-type filters clarify the schedule without conflating the ship-by deadline with the later pickup window. The view is read-only for ordinary Executive sessions; Branch-tier credentials unlock versioned day/hour and handover-method rules that API Ingest reads on its two-minute Hostinger worker. Instant stays manual-only and the marketplace continues to choose the actual available pickup slot.
