...
- Hard Set is initialized server-side as OFF. First activation remains irreversible: the UTC cutover timestamp and exact account scope cannot be changed. Automatic marketplace shipment arrangement is permanently pause-locked; activation begins paused, resume requests are rejected, and any legacy unpaused state is repaired and projected downstream. The lock never reverses an arranged shipment, stops label recovery, disables the manual Instant action, or affects Partner orders.
