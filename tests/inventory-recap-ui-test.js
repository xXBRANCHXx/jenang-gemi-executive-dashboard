...
assert.match(script, /temporary_mark_paid_eligible[\s\S]*data-po-mark-historical-paid[\s\S]*Mark paid/);
assert.match(script, /markHistoricalPoPaid[\s\S]*action: 'mark_historical_paid'/);
assert.match(api, /mark_historical_paid[\s\S]*jg_purchase_orders_mark_historical_paid/);
assert.match(purchaseOrders, /historical-paid-reconciliation-[\s\S]*historical_full/);
assert.match(styles, /\.admin-po-temporary-paid[\s\S]*#15803d/);
