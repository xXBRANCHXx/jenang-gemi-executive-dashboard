...
assert.match(dashboard, /data-po-payment-proof[\s\S]*PDF, PNG, JPG, or WebP/);
assert.match(script, /data-po-tag/);
assert.match(script, /payment\.proof\?\.url[\s\S]*Open proof/);
assert.match(script, /new FormData\(\)[\s\S]*form\.set\('proof', proof\)/);
...
assert.match(api, /multipart\/form-data[\s\S]*jg_purchase_orders_validate_payment_proof/);
