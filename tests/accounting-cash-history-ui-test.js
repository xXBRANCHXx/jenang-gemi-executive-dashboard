...
expect(html.includes('data-accounting-cash-history-balance-class'), 'Balance history must split bank and physical cash.');
expect(html.includes('data-accounting-cash-history-account'), 'Cash history must expose an account dropdown.');
expect(html.includes('value="bank">Bank balance</option>'), 'Balance history must offer the bank scope.');
expect(html.includes('value="cash">Available cash</option>'), 'Balance history must offer the physical cash scope.');
expect(!html.includes('data-accounting-cash-history-platform'), 'Source platforms must not blur the bank-versus-cash distinction.');
expect(script.includes('populateCashHistoryAccounts'), 'Balance history must populate real operational account filters.');
expect(script.includes('filteredSummary.current_cash'), 'Cash history must recalculate platform-and-account summary totals.');
expect(script.includes("'Bank balance'") && script.includes("'Available cash'"), 'Filtered history must use explicit balance labels.');
expect(script.includes('filteredRunningBalances'), 'Cash history must recalculate running balances for the selected platform and account.');
