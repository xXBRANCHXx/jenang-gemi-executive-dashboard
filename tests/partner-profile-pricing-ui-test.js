...
assert.match(page, /name="billing_period_type"[\s\S]*Calendar Week \(Monday–Sunday\)[\s\S]*Calendar Month/, 'Partner profiles must expose both supported billing-period choices.');
assert.match(script, /billing_period_type\.value = partner\.billing_period_type === 'calendar_month' \? 'calendar_month' : 'calendar_week'/, 'Existing and unset profiles must render Calendar Week by default.');
