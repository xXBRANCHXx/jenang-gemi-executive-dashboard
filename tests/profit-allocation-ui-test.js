...
assert.match(page, /Selected month:[\s\S]*data-pnl-allocation-period/, 'Allocation settings must identify the selected month.');
...
assert.match(script, /year: state\.year, month, allocation_tree:/, 'Allocation settings must save against a specific month.');
assert.match(script, /month=\$\{requestedMonth\}[^`]*scope=allocation_settings/, 'Allocation settings must load against the selected month.');
